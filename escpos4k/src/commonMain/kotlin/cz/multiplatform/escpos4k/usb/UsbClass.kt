...
public sealed class UsbClass(internal val raw: UByte) {
   ...
   *
   * This base class is defined to be used in Device Descriptors to indicate that class information
   * should be determined from the Interface Descriptors in the device. There is one class code
   * definition in this base class. All other values are reserved.
   *
   ...
   *
   * This base class is defined for Audio capable devices that conform to the Audio Device Class
   * Specification found on the USB-IF website. That specification defines the usable set of
   * SubClass and Protocol values. Values outside that defined spec are reserved. These class codes
   ...
   * Specification found on the USB-IF website. That specification defines the usable set of
   * SubClass and Protocol values. Values outside of that defined spec are reserved. Note that the
   * Communication Device Class spec requires some class code values (triples) to be used in Device
   ...
   *
   * This base class is defined for devices that conform to the HID Device Class Specification found
   * on the USB-IF website. That specification defines the usable set of SubClass and Protocol
   ...
   * found on the USB-IF website. That specification defines the usable set of SubClass and Protocol
   * values. Values outside of that defined spec are reserved. These class codes can only be used in
   * Interface Descriptors.
   */
   ...
   *
   * This base class is defined for devices that conform to the Imaging Device Class Specification
   * found on the USB-IF website. That specification defines the usable set of SubClass and Protocol
   * values. Values outside of that defined spec are reserved.
   */
   ...
   * Specification found on the USB-IF website. That specification defines the usable set of
   * SubClass and Protocol values. Values outside of that defined spec are reserved. These class
   ...
   * Base Class 09h (Hub)
   *
   * This base class is defined for devices that are USB hubs and conform to the definition in the
   * USB specification. That specification defines the complete triples as shown below. All other
   * values are reserved. These class codes can only be used in Device Descriptors.
   */
   ...
   * Specification found on the USB-IF website. That specification defines the usable set of
   * SubClass and Protocol values. Values outside of that defined spec are reserved. These class
   * codes should only be used in Interface Descriptors.
   */
   ...
   *
   * The USB Audio/Video (AV) Device Class Definition describes the methods used to communicate with
   * devices or functions embedded in composite devices that are used to manipulate audio, video,
   * voice, and all image- and sound-related functionality. That specification defines the usable
   * set of SubClass and Protocol values. Values outside of that defined spec are reserved. These
   ...
   * found on this USB-IF website. That specification defines the usable set of SubClass and
   * Protocol values. Values outside of that defined spec are reserved. These class codes can only
   * be used in Interface Descriptions.
   */
   ...
   *
   * This base class is defined for devices that diagnostic devices. This class code can be used in
   * Device or Interface Descriptors. Trace is a form of debugging where processor or system
   * activity is made externally visible in real-time or stored and later retrieved for viewing by
   * an applications developer, applications program, or, external equipment specializing observing
   * system activity. Design for Debug or Test (Dfx). This refers to a logic block that provides
   * debug or test support (E.g. via Test Access Port (TAP)). DvC: Debug Capability on the USB
   ...
   *
   * This base class is defined for devices that are Wireless controllers. Values not shown in the
   * table below are reserved. These class codes are to be used in Interface Descriptors, with the
   * exception of the Bluetooth class code which can also be used in a Device Descriptor.
   */
   ...
   *
   * This base class is defined for miscellaneous device definitions. Values not shown in the table
   * below are reserved. The use of these class codes (Device or Interface descriptor) are
   * specifically annotated in each entry below.
   */
   ...
   *
   * This base class is defined for devices that conform to several class specifications found on
   * the USB-IF website. That specification defines the usable set of SubClass and Protocol values.
   * Values outside of that defined spec are reserved. These class codes can only be used in
   * Interface Descriptors.
   */
   ...
   *
   * This base class is defined for vendors to use as they please. These class codes can be used in
   * both Device and Interface Descriptors.
   */
